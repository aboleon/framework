...
class MediaUploader extends Model
{
    ...
    protected function image(object $object)
    {
        if (request()->has('files')) {
            $this->uploaded_file = request()->file('files')[0];
        }

        $this->uploadPath = $object->uploadParams['path'];
        $this->checkMakeDir();
        ...
        if (!is_null($this->uploaded_file) && !request()->filled($this->uploaded_filename . '_upload_errors')) {
            ...
            $this->processSingleSizeImage($object);
        ...
        // Thumb
        $this->generateThumbnail($object);

        $this->response['filename'] = $this->prefix . $this->randomName . '.' . $this->mime_type;
    ...
    protected function document(object $object): void
    {
        $this->uploadable_type = 'document';

        if (!is_null($this->uploaded_file)) {
            $path = $this->uploaded_file->store($object->uploadParams['docs'], 'public');

            $newfilename = explode('/', $path);

            $this->response['path'] = $path;
            $this->response['newfilename'] = end($newfilename);
            $this->response['http'] = $path;
        }
    }
/*
...
*/
    ...
    private function generateThumbnail(object $object)
    {
        ...
        $this->img_object->encode('jpg')->resize(null, 110, function ($constraint) {
            $constraint->aspectRatio();
            $constraint->upsize();
        })->save($this->uploadPath . 'th_' . $this->prefix .
            $this->randomName .
            '.' . $this->mime_type, 75);
    }
    ...
    public function jcrop(): void
    {
        ...
        $this->img_object = Image::make($this->uploadPath . $image->content)->crop(
        ...
        if ($this->img_object->save($this->uploadPath . $composants[0] . '@2x.' .
            $composants[1], 75)) {

            $img2 = Image::make($this->uploadPath . $image->content)->crop(
                (int)request()->{$image->varname . '_wimage'},
                (int)request()->{$image->varname . '_himage'},
                (int)request()->{$image->varname . '_x1image'},
                (int)request()->{$image->varname . '_y1image'}
            ...
            $img2->save($this->uploadPath . $clean_name, 75);

            Image::make($this->uploadPath . $clean_name)->resize(null, 110, function ($constraint) {
                $constraint->aspectRatio();
                $constraint->upsize();
            })->save($this->uploadPath . 'th_' . $clean_name, 80);

            File::delete(File::glob($this->uploadPath . '*' . str_replace(['.jpg', '.png'], '', $image->content) . '*'));
        }
    ...
    private function processSingleSizeImage(object $object)
    {
        if (!array_key_exists('sizes', $this->uploaded_file_config)) {
            /* Image retina */

            $this->img_object->encode('jpg')->save($this->uploadPath . '/' .
                $this->prefix .
            ...
            $this->img_object->resize($this->resize_by_w, $this->resize_by_h, function ($constraint) {
                $constraint->aspectRatio();
                $constraint->upsize();
            })->save($this->uploadPath . $this->prefix .
                $this->randomName
                . '.' . $this->mime_type, 75);
        }
    }
    ...
    private function processSizes()
    {
        if (array_key_exists('sizes', $this->uploaded_file_config)) {
            ...
            foreach ($this->uploaded_file_config['sizes'] as $isize) {
                ...
                $this->img_object->save($this->uploadPath . '/images/' .
                    $this->randomName
