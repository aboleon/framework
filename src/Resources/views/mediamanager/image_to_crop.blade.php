@php
    $resized_img = list($temp_width, $temp_height) = getimagesize(public_path($data->uploadParams['dir']  . $uploadedImage->content));
@endphp
...
<div>
    <img alt="Image à recadrer"
         src="{!! $data->uploadParams['dir']  . $uploadedImage->content) !!}"
         class="jcrop_image"/>
</div>
<div class="form-group mt-20">
    <a href="{!! url(config('aboleon_framework.route').'/mediamanager/remove/' . $uploadedImage->id) !!}"
       class="btn btn-danger">
        <i class="white fas fa-trash-alt"></i> Supprimer la photo
    </a>
    <button class="btn btn-warning" name="{!! $image_key !!}_jcrop_confirm"
            type="submit">
        <i class="fas fa-ok bigger-110"></i> Valider le recadrage
    </button>
